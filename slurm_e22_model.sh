...
model="$NAR_E22_MODEL"
run() { "$python_bin" "$code_dir/nar/e22_qwen3_family.py" --workdir "$NAR_WORKDIR" --model "$model" \
...
for stage in ${NAR_E22_STAGES:-audit calibrate control gptq eval finalize}; do
    ...
    case "$stage" in
        ...
        eval)      for benchmark in ${NAR_E22_BENCHMARKS:-wikitext c4 mmlu gsm8k mmlu_redux gpqa arc_easy math}; do
