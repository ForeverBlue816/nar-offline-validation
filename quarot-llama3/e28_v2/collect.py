...
def main():
    ...
    if (out/'initialization_revision.json').exists():
        ...
        note=f'## Initialization revision and reused evidence\n\nThe original1..6 packed-byte initialization leaves half the channels zero and all other weights positive. Its8B NAR model produced nonfinite outputs: the last decoder residual overflowed FP16 although the R4 operators stayed finite. [Original failure and partial measurements](../{prior}/report_e28_v2.md) are retained. This run uses one predefined uniform0..255 packed-byte initialization for both integer methods, with unchanged scales/seeds/kernels/thresholds. It is a separate frozen random-weight protocol, not a claim of improved model quality.\n\n[Reused correctness records](reused_correctness.json) identify unchanged local operators/factors and FP16 states verified in the same A40 allocation. All four integer model paths are rechecked for the new state; all formal performance sessions are newly measured.\n\n'
