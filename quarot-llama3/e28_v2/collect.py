...
def main():
    ...
    for model in MODELS:
      for mode in ['eager_sequence','eager_step_sync','cuda_graph_sequence']:
       for phase in ['prefill1','prefill16','decode','decode8','decode8192']:
        ...
        fp16_comparison={}
        if rows['fp16']:
            fs={r['session']:r['median'] for r in rows['fp16']['session_summaries']}
            for method in ['hadamard','nar']:
                if not rows[method]:continue
                ms={r['session']:r['median'] for r in rows[method]['session_summaries']}
                paired=[{'session':session,'speedup':fs[session]/ms[session] if decode else ms[session]/fs[session]} for session in sorted(fs.keys()&ms.keys())]
                ratios=[r['speedup'] for r in paired]
                consistent=len(ratios)==3 and (all(v>1 for v in ratios) or all(v<1 for v in ratios))
                pooled=sh if method=='hadamard' else sn
                interpretation=('fewer than three comparable sessions' if len(ratios)<3 else
                                'near parity / within observed variability' if not consistent or abs(pooled-1)<=.01 else
                                'faster than FP16 in all three sessions' if pooled>1 else 'slower than FP16 in all three sessions')
                fp16_comparison[method]={'session_speedups':paired,'direction_consistent':consistent,'interpretation':interpretation}
        c['fp16_comparison']=fp16_comparison
    ...
    def check_status(name, record):
        if record.get('complete') is False:return 'RUNNING'
        if record.get('status'):return record['status']
        return 'DIAGNOSTIC_RECORDED' if name=='cache_diagnose' and record.get('pairs') else 'MISSING_STATUS'
    summary={'generated_at':now(),'protocol_sha256':sha(out/'protocol.json'),'deployment':aggregate,'comparisons':comparisons,'correctness_status':{k:{**{t:v.get(t) for t in ['reason','complete','completed_layers']},'status':check_status(k,v)} for k,v in correct.items()},
    ...
    for model in MODELS:
      for method in METHODS:
        ...
        for phase in ['prefill16','decode']:
            measured=get(model,method,'eager_sequence',phase)
            hd=get(model,'hadamard','eager_sequence',phase);fp=get(model,'fp16','eager_sequence',phase)
            mem.append([model,names[method],phase,fmt(measured['peak_allocated_bytes']/1e9 if measured else None),fmt((measured['peak_allocated_bytes']-hd['peak_allocated_bytes'])/1e6 if measured and hd and method=='nar' else None),fmt(100*(1-measured['peak_allocated_bytes']/fp['peak_allocated_bytes']) if measured and fp else None)])
    ...
    table(out/'tables/memory',['Model','Method','Phase','Peak GB','Extra vs Had MB','Saved vs FP16 %'],mem,'Independent process peak allocated bytes, GB=1e9. Reserved and NVML values are reported separately in JSON.')
    kernels=[]
    for r in raw:
    ...
    real_status='/'.join(correct.get('model_real_'+model+'_fp16',{}).get('status','PENDING') for model in MODELS)
    comp=[['E28 kernel swap','Sym W4/A4; KV4','Random','Boundary FAIL','Measured integer'],['FP16 base models','W16/A16/KV16','Real base','3B/8B: '+real_status,'FP16 only'],['E17 native R4','Asym A4 g128','Real factors','See kernel table','Local R4 only'],['Paper native','Asym g128; KV R32','k8 state absent','BLOCKED','GEMM/KV gap']]
    ...
    (out/'paper/experiment.tex').write_text('We benchmark random-weight Llama-3.2-3B and Llama-3.1-8B on one NVIDIA A40 with the existing QuaRot integer backend. Core measurements use ten complete warm-up runs and fifty repetitions in each of three balanced sessions. Decode is a sequence of causal model-forward calls, excludes the first eight of 128 steps, and is not serving or sampling latency. We report independent process memory and separate wall, CUDA-event and profiler durations. Per-session and pooled statistics are generated from raw JSON; missing or invalid results are not imputed.\n')
    ...
    statuslines=[f'| {k} | {check_status(k,v)} | {v.get("reason", "")} |' for k,v in sorted(correct.items())]
    ...
    for c in comparisons:
        if c['mode']=='eager_sequence' and c['phase'] in ['prefill1','prefill16','decode']:
            fp_interpretation='; '.join(f"{method}: {entry['interpretation']}" for method,entry in c['fp16_comparison'].items())
            answer.append(f"- {c['model']} {c['phase']}: Hadamard/FP16 speedup {fmt(c['hadamard_speedup'])}x; PrismQuant/FP16 {fmt(c['nar_speedup'])}x. {fp_interpretation}.")
            relative=(f"NAR latency difference {fmt(c['nar_latency_overhead_pct'])}%" if c['phase']=='decode' else f"NAR throughput is {fmt(c['nar_throughput_ratio_pct'])}% of Hadamard")
            hadamard_answer.append(f"- {c['model']} {c['phase']}: {relative}; {c.get('interpretation','comparison incomplete')}.")
    ...
    dispatch_answer=[]
    for row in kernel_evidence['comparisons']:
        if row['scope']=='Dispatch':
            dispatch_answer.append(f"- {row['model']} T={row['tokens']}: prebound/generic wall ratio {fmt(row['wall_ratio']['median'])}, CUDA-event elapsed ratio {fmt(row['event_ratio']['median'])}; {row['interpretation']}.")
    profile_answer=[]
    profile_evidence=read(out/'profile_summary.json')['rows']
    for model in MODELS:
        pair={row['method']:row for row in profile_evidence if row['model']==model}
        had=pair.get('hadamard',{}).get('kernel_sum_ms_per_step');nar=pair.get('nar',{}).get('kernel_sum_ms_per_step')
        if had is not None and nar is not None:
            profile_answer.append(f"- {model}: separate profiled NAR-minus-Hadamard kernel sum {fmt(nar-had)} ms/step. {'This trace does not show reduced total GPU kernel duration.' if nar>=had else 'This diagnostic trace shows lower summed kernel duration.'} Profiler perturbation and host submission prevent interpreting this as a causal explanation of the main wall-time difference.")
    def graph_count(prefix):
        rows=[correct.get(f'{prefix}{model}_{method}',{}) for model in MODELS for method in METHODS]
        return f"{sum(r.get('status')=='PASS' for r in rows)}/6 PASS, {sum(r.get('status') in ('FAIL','INVALID','BLOCKED') for r in rows)}/6 failed or blocked, {sum(not r or r.get('status')=='RUNNING' for r in rows)}/6 pending"
    graph_answer=f"Original backend: {graph_count('graph_')}. Private current-stream backend: {graph_count('graph_stream_')}. Matched private timing panel: {summary['graph_performance'].get('status','INCOMPLETE')}."

    report=f'''# E28-v2: auditable deployment experiments\n\nGenerated {now()}. This report contains completed records only where the linked JSON says COMPLETE/PASS; other stages remain explicitly incomplete, failed or blocked. All model performance rows use **random weights**, even when validation inputs are real text. No full model-quality evaluation is claimed.\n\n## One-page status\n\nCore timing: **{progress['core_complete']}/72** independent processes complete. Full timing completion does not override any numerical failure below. The [stage completion table](tables/completion.md) separately tracks every P1/P2 check and timing panel.\n\n- Preserved original E28/E17/E22/E26 artifacts.\n- Froze counts, shapes, selected configs, thresholds and exclusions in [protocol.json](protocol.json); execution provenance in [source_manifest.json](source_manifest.json) and [execution_source_manifest.json](execution_source_manifest.json).\n- Corrected shared INT4 initialization, stale RoPE caches, timing boundaries, complete cache reset, throughput statistics, independent memory and invalid acceleration-retention ratios. The common metadata optimization is checked against the original wrapper with the same rebuilt RoPE caches on both sides; this does not establish equivalence to historical stale-RoPE outputs.\n- The exact original environment and A40 are reused; no new GEMM, calibration or serving framework.\n- Native paper-format integer deployment is BLOCKED by missing matching complete checkpoint and incompatible group-scale/KV interfaces.\n\n| Check | Status | Reason |\n| --- | --- | --- |\n{chr(10).join(statuslines)}\n\n## Numerical boundaries and real weights\n\n{boundary_text or 'No failing operator boundary row has been recorded yet.'}\n\nReal base-checkpoint FP16 implementation checks: {real_text}. The independent pinned RoPE reference is recorded in `correctness/rope_reference_3b.json` and `correctness/rope_reference_8b.json` when run. This is separate from compensated INT4 checkpoint support and from model quality. Prefill hidden/logits and multi-step decode are checked; exact cache comparisons cover steps 1, 2, 9, 64, 65, 128.\n\n## Six questions\n\n1. **Which stages accelerate relative to FP16?** Ratios above 1x indicate faster execution than FP16; ratios below 1x indicate slower execution. Same-mode ratios only; [raw metrics and session status](metrics_summary.json), [deployment prefill](tables/deployment_prefill.md), [decode](tables/deployment_decode.md).\n{chr(10).join(answer) or '- No complete measurements yet; no speedup claim.'}\n\n2. **What does PrismQuant add versus Hadamard?** See directly measured A/B/quantizer/chain times in [kernel table](tables/kernel.md), same-mode wall ratios in [metrics](metrics_summary.json), and exact unique-storage categories/deltas in [memory_breakdown.json](memory_breakdown.json) and [memory_deltas.json](memory_deltas.json). Factor storage is 96d bytes per layer plus one shared 32768-byte H128; scratch is recorded separately. No rounded-to-zero overhead claim.\n{chr(10).join(hadamard_answer)}\n{chr(10).join(profile_answer)}\n\n3. **Does dispatch optimization help reproducibly?** The generic/prebound protocol requires identical kernels, byte-equal outputs and three separately recorded sessions; only completed valid records support a conclusion. See [kernel records](tables/kernel.md). A one-percent difference without consistent session direction is near parity, not a stable advantage.\n{chr(10).join(dispatch_answer)}\n\n4. **Does graph replay grow the KV context?** See [graphability audit](graphability_audit.md) and the graph correctness rows above. Only A-B-A replay with steps 1, 2, 9, 64, 65, 128, full cache hashes and actual page crossing can pass. Original-binary capture and the bounded private current-stream adapter have separate records. Any matched adapter timing is in the [separate graph panel](tables/graph_deployment.md), with [individual session results](tables/graph_sessions.md), [preparation/memory table](tables/graph_memory.md) and [allocation details](graph_memory_breakdown.json). The timed layout uses one preallocated 2176-token page for prefix 2048 plus 128 causal steps; a separate prefix-128/page-64 correctness layout crosses actual page boundaries. Capture deltas are global allocator changes, not isolated private-pool sizes; standalone pool attribution remains unavailable. No graph performance panel is populated from a fixed-context microbenchmark; absent timings remain null. {graph_answer}\n\n5. **Which results execute integer kernels?** E28 Hadamard/NAR use the actual packed INT4 GEMM/quantizer and INT4 decode cache; FP16 uses FP16 arithmetic/cache. E17-native rows are local packed-activation microbenchmarks, not an end-to-end model. Random states are verified by [shared_state_audit.json](shared_state_audit.json). Real model quality remains untested.\n\n6. **What is missing for the paper's group128 asymmetric path?** See [quantizer contract](quantizer_contract.md) and [checkpoint audit](checkpoint_audit.json). Per-group reduction scales/offsets need partial sums/corrections absent from this GEMM interface; token/channel KV grouping and residual windows also differ. No discarded offsets or FP16 substitute is presented as native integer deployment.\n\n## Technical appendix\n\n[Kernel design](kernel_design.md), [old claims audit](old_claim_audit.json), [compatibility table](tables/compatibility.md), [memory table](tables/memory.md), and [measured evidence appendix](evidence_appendix.md), [profile summary](profile_summary.json) and [profile traces](profiles/) provide the detailed evidence. `correctness/attempt_1/` retains initial harness failures; [harness corrections](verification_harness_corrections.json) explain the repair without changing numerical thresholds.\n\nAll tables are regenerated by `collect`, which never launches a GPU experiment. Memory is bytes/decimal GB; pooled 150 runs represent 3 sessions. Profiler kernel sums, CUDA-event elapsed and wall time are distinct. Missing counters mean bandwidth/compute bottleneck labels remain hypotheses.\n'''
