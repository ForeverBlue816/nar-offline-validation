...
def main():
    ...
    for c in comparisons:
        if c['mode']=='eager_sequence' and c['phase'] in ['prefill1','prefill16','decode']:
            ...
            if complete:
                paper_results.append(f"For {MODELS[c['model']].replace('_','-')} {c['phase']}, Hadamard and PrismQuant achieve {fmt(c['hadamard_speedup'])}x and {fmt(c['nar_speedup'])}x relative to FP16 under the same eager-sequence timing protocol. {c.get('interpretation','')}. ")
