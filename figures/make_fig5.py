...
def main():
    ...
    export(fig,HERE/'fig5',draft=args.draft,exclude_axes=[strip],panel_axes={**dict(zip('abcd',axes)),'implementation':strip})
    meta=json.loads((HERE/'deployment_efficiency_metadata.json').read_text());meta['typography']=typography
    meta['size_inches']=[5.5,5.35]
    meta['figure_number']=5;meta['canonical_export']='fig5';meta['data_file']='deployment_efficiency_data.csv'
    ...
    (HERE/'fig5_metadata.json').write_text(json.dumps(meta,indent=2)+'\n')
