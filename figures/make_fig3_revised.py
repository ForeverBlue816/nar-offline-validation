...
BURGUNDY='#601D49';BIN_FILL='#F2E4ED';LILAC='#A99DBB'
...
def energy(ax,d,appendix=False):
    axis(ax,'a' if appendix else 'b','Energy across depth','Llama-3.2-3B · seven measured layers' if appendix else 'Solid: non-BOS · dashed: all tokens')
    ax.grid(False)
...
def experts(ax,d,summary,stats):
    ...
    ax.vlines(b.x_median,b.y_q1,b.y_q3,color=BURGUNDY,alpha=.58,lw=2.5,zorder=6)
    ax.plot(b.x_median,b.y_median,color=BURGUNDY,lw=2.1,marker='o',ms=3.9,mfc=BIN_FILL,mec=BURGUNDY,mew=.8,zorder=7)
...
def law_legend(fig):
    handles=[Line2D([],[],marker=m,color=color,ls='',ms=4.6,label=name)
        for name,color,m in [('Activations',STEEL,'o'),('V-cache',TEAL_EDGE,'s'),('Multi-slot',LILAC,'D')]]
    handles += [Line2D([],[],color=BLUE,lw=2.3,label='Dense reference'),
        Line2D([],[],color=BURGUNDY,marker='o',ms=3.9,mfc=BIN_FILL,mec=BURGUNDY,mew=.8,lw=2.1,label='Bin median + IQR')]
    fig.legend(handles=handles,loc='center',bbox_to_anchor=(.55,.505),ncol=5,frameon=False,
        fontsize=7.5,handlelength=1.7,columnspacing=1.4,handletextpad=.45,borderpad=0)
...
def main():
    ...
    fig,axes=plt.subplots(2,2,figsize=(6.6,4.95));fig.subplots_adjust(left=.095,right=.98,bottom=.135,top=.885,wspace=.34,hspace=.85)
    a,b,c,e=axes.flat
    ...
    meta=dict(source_commit=source['source_commit'],size_inches=[6.6,4.95],panels=list('abcd'),typography=typography,
        panel_point_counts=source['count_fig3_by_panel'],energy_layers=LAYERS,energy_protocols=source['energy_protocols'],
        shared_legend={'location':'between panel rows','center_figure_fraction':[.55,.505],'columns':5},
        bin_summary_color=BURGUNDY,bin_marker_fill=BIN_FILL,reference_fit=FIT,moe_diagnostics=source['moe_diagnostics'],moe_binning=source['binning'],
