...
def caption_export(text,number):
    ...
    fig=plt.figure(figsize=(6.6,2),dpi=150);renderer=fig.canvas.get_renderer()
    lines=[];line=''
    for word in text.split():
        trial=f'{line} {word}'.strip()
        if line and renderer.get_text_width_height_descent(trial,prop,False)[0]>(6.6-.32)*fig.dpi:
            lines.append(line);line=word
        else:line=trial
    lines.append(line)
    height=.32+len(lines)*8*1.35/72;fig.set_size_inches(6.6,height)
    ax=fig.add_axes([.16/6.6,.16/height,1-.32/6.6,1-.32/height]);ax.axis('off')
...
def main():
    ...
    (HERE/'include_fig2_fig3.tex').write_text(r'''% Wider Figures 2/3 are designed at 6.6 inches with >=7.5-pt glyphs.
