...
WARM='#BC9272';WARM_FILL='#E1C9B5'
METHODS=[('hadamard','Hadamard',TEAL_EDGE,TEAL,'o'),
         ('duquant_style','DuQuant',WARM,WARM_FILL,'D'),('nar','PrismQuant',BLUE,BLUE,'s')]
...
def main():
    ...
    typography=style();data=pd.read_csv(HERE/'fig2_revised_data.csv');source=json.loads((HERE/'fig2_fig3_source_metadata.json').read_text())
    ...
    for ax,letter,title,ylabel in zip(axes,'abc',titles,ylabs):
        ...
        for method,label,color,fill,marker in METHODS:
            ...
            ax.plot(part.x,part.y,color=color,lw=2.3 if method=='nar' else 1.85,
                marker=marker,ms=3.25 if method=='nar' else 2.9,mfc=fill,mec=color,mew=.65,label=label,zorder=5 if method=='nar' else 3)
        if letter=='a':
            ax.axhline(1/128,color=GRAY_EDGE,lw=1.5,ls=(0,(3,2)),zorder=0)
    ...
    handles=[Line2D([],[],color=color,marker=marker,mfc=fill,mec=color,mew=.65,lw=2.3 if method=='nar' else 1.85,ms=3.8,label=label)
    ...
    meta=dict(source_commit=source['source_commit'],size_inches=[5.5,2.55],panels=list('abc'),typography=typography,
        ...
        line_width_pt={'PrismQuant':2.3,'comparators':1.85,'reference':1.5},
        minimum_font_pt=7.5,palette={'PrismQuant':BLUE,'Hadamard':TEAL_EDGE,'DuQuant':WARM},
