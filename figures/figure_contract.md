...
Backend: Python/matplotlib, Times New Roman Bold. Figures 2/3 now use 6.6-inch native width; Figures 4/5 retain 5.5 inches.

Figure 4 asks how activation metadata and rank affect PPL. It retains all 11 budget points plus BF16 (3B main, 8B appendix), all 15 rank points and the restored original decoder-layer cost panel. A wider green band marks the k=8 category only. The former Graph-overhead panel is archived; its complete source values remain in the CSV and Figure 5 evidence.
...
and bulk R4 implementation choices. The user has confirmed that deployment efficiency is Figure 5; canonical exports and manuscript references now use fig5.
...
Figures 4/5 remain 5.5 inches wide; the latest Figure 2/3 dimensions are recorded below. All figures use true embedded Times New Roman Bold, 7.5 pt minimum glyph size, 8.5 pt axis labels, 10.5 pt panel letters, blue/teal primary encoding, 2.1–2.3 pt main curves, editable vector PDF/SVG and 600-dpi PNG. Every export must pass 1.5-pt rendered alignment, clipping, collisions and actual PDF font audits. No experiments, GPU runs, model calibration or new fitted line is authorized or needed.
...
## Wider Figures 2/3 and confirmed Figure 5 numbering

The user requests a larger horizontal-to-vertical ratio. Keep the existing paired-layer and four-panel scientific arguments, all data coordinates, original Fig2 pastel colors/thin strokes and minimum 7.5-pt text. Render Figure 2 at 6.6 × 2.25 inches and Figure 3 at 6.6 × 4.95 inches (overriding the earlier 5.5-inch width only for these two figures). Put the shared reference/family/bin legend between rows b and c instead of below the bottom row. Use Figure 1 burgundy #601D49 for bin medians, marker edges and quartile intervals; this color encodes a descriptive summary, not significance. Update native-size captions and inclusion notes.

The user has now assigned deployment efficiency to Figure 5. Canonical exports, component panels, metadata, caption key and LaTeX label must use fig5. Retain the frozen deployment data and provenance filenames, original 5.5 × 5.35-inch layout and all measurement semantics. The old Python generator remains a forwarding entry point. All comparable plot rectangles and labels must pass 1.5-pt alignment, full-point clipping, actual embedded Times fonts, PDF/SVG independent renders, and final collision audits.
