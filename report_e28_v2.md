...
| graph_stream_3b_hadamard | PASS |  |
| graph_stream_3b_nar | PASS |  |
| graph_stream_8b_fp16 | PASS |  |
| graph_stream_8b_hadamard | PASS |  |
| graph_stream_8b_nar | PASS |  |
| model_3b_fp16 | PASS |  |
...
1. **Which stages accelerate relative to FP16?** Ratios above 1x indicate faster execution than FP16; ratios below 1x indicate slower execution. Same-mode ratios only; [raw metrics and session status](results/e28_v2/20260912_a40_v2_full_int4/metrics_summary.json), [deployment prefill](results/e28_v2/20260912_a40_v2_full_int4/tables/deployment_prefill.md), [decode](results/e28_v2/20260912_a40_v2_full_int4/tables/deployment_decode.md).
...
- Private Graph panel, 3b hadamard: 0.92x relative to the same-mode private FP16 baseline; consistent observed direction in these three sessions.
- Private Graph panel, 3b nar: 0.90x relative to the same-mode private FP16 baseline; consistent observed direction in these three sessions.
- Private Graph panel, 8b hadamard: 1.25x relative to the same-mode private FP16 baseline; consistent observed direction in these three sessions.
- Private Graph panel, 8b nar: 1.22x relative to the same-mode private FP16 baseline; consistent observed direction in these three sessions.

2. **What does PrismQuant add versus Hadamard?** See directly measured A/B/quantizer/chain times in [kernel table](results/e28_v2/20260912_a40_v2_full_int4/tables/kernel.md), same-mode wall ratios in [metrics](results/e28_v2/20260912_a40_v2_full_int4/metrics_summary.json), and exact unique-storage categories/deltas in [memory_breakdown.json](results/e28_v2/20260912_a40_v2_full_int4/memory_breakdown.json) and [memory_deltas.json](results/e28_v2/20260912_a40_v2_full_int4/memory_deltas.json). Factor storage is 96d bytes per layer plus one shared 32768-byte H128; scratch is recorded separately. No rounded-to-zero overhead claim.
...
- Private Graph panel, 3b: NAR latency difference 1.96% versus same-mode private Hadamard; consistent observed direction in these three sessions.
- Private Graph panel, 8b: NAR latency difference 2.35% versus same-mode private Hadamard; consistent observed direction in these three sessions.
...
4. **Does graph replay grow the KV context?** See [graphability audit](results/e28_v2/20260912_a40_v2_full_int4/graphability_audit.md) and the graph correctness rows above. Only A-B-A replay with steps 1, 2, 9, 64, 65, 128, full cache hashes and actual page crossing can pass. Original-binary capture and the bounded private current-stream adapter have separate records. Any matched adapter timing is in the [separate graph panel](results/e28_v2/20260912_a40_v2_full_int4/tables/graph_deployment.md), with [individual session results](results/e28_v2/20260912_a40_v2_full_int4/tables/graph_sessions.md), [preparation/memory table](results/e28_v2/20260912_a40_v2_full_int4/tables/graph_memory.md) and [allocation details](results/e28_v2/20260912_a40_v2_full_int4/graph_memory_breakdown.json). The timed layout uses one preallocated 2176-token page for prefix 2048 plus 128 causal steps; a separate prefix-128/page-64 correctness layout crosses actual page boundaries. Capture deltas are global allocator changes, not isolated private-pool sizes; standalone pool attribution remains unavailable. No graph performance panel is populated from a fixed-context microbenchmark; absent timings remain null. Original backend: 0/6 PASS, 6/6 failed or blocked, 0/6 pending. Private current-stream backend: 6/6 PASS, 0/6 failed or blocked, 0/6 pending. Matched private timing panel: COMPLETE.
