...
def build(args: argparse.Namespace) -> None:
    ...
    section = [
        ...
        "Every row uses post-RoPE KIVI-style K4: dynamic asymmetric per-channel quantization over contiguous 32-token groups; an incomplete newest group stays bf16. This replaces QuaRot R3/per-token K in all rows. V4 is dynamic asymmetric per-token over one 128-channel head group. Hadamard rows use QuaRot's random-sign R1 and exact unsigned R2/R4; the NAR row uses calibrated global R1, per-layer per-head R2, and per-layer R4.\n",
        "The first row preserves upstream symmetric per-token A4 semantics. The other two quantize inputs to q/k/v/o/gate/up/down with fp16-scale/fp16-offset asymmetric group-128 A4. The same seed, GPTQ calibration chunks, full contiguous WikiText-2 test stream, harness revision, prompts, tasks, and metrics are paired. This is a frozen one-seed end-to-end check, so no confidence interval is claimed.\n",
