...
def build(args: argparse.Namespace) -> None:
    ...
    section = [
        ...
        (
            ...
            "Cayley/Stiefel optimization is run. The primary reference is community reproduction "
            "feedback in the official Facebook Research repository, rather than the paper headline. "
            "In [Issue #11](https://github.com/facebookresearch/SpinQuant/issues/11), the reported "
            "Llama3-70B GPTQ/W4A4KV4 WikiText-2 PPL is 7.5821 versus 4.1 in the paper "
            "(+3.4821). [Issue #42](https://github.com/facebookresearch/SpinQuant/issues/42) "
            "reports downstream ARC-Easy accuracy "
            "65.40% ± 0.98% versus 72.6% (-7.20 percentage points) and ARC-Challenge "
            "37.03% ± 1.41% versus 47.5% (-10.47 points). The developer raised calibration-set "
            "overfitting as a possible explanation; this is retained as a hypothesis, not a "
            "demonstrated cause. Community feedback also reports close RTN reproduction for "
            "Llama2-7B/13B/70B and Llama3-8B, while [Issue #50]"
            "(https://github.com/facebookresearch/SpinQuant/issues/50) records an additional "
            "Llama3-8B PPL mismatch. These are quoted community results, not measurements from "
            "this repository. Every other baseline whose method requires training follows the "
            "released-artifact-or-published-number rule and is never trained locally.\n\n"
            "*Footnote:* These reports concern Llama3-70B or Llama3-8B, not the present "
